...
export default async function SpecialApplicationDetailPage({
  ...
  const answers = {
    ...
    chair: formatChairAssistantChair(application.chair),
    undergraduateInfo: application.undergraduateInfo,
    graduateProgramInfo: application.graduateProgramInfo,
    academicFields: application.academicFields,
    thesisTopic: application.thesisTopic || '-',
    ...
    attendanceCommitment: application.attendanceCommitment,
    additionalNotes: application.additionalNotes || '-',
    accuracyDeclarationAccepted: application.accuracyDeclarationAccepted ? 'Evet' : 'Hayır',
  } satisfies Record<(typeof chairAssistantQuestions)[number]['key'], string>
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-semibold">{application.fullName}</h1>
            <div className="flex flex-wrap items-center gap-3 text-sm text-slate-400">
              <span>{format(application.createdAt, 'dd.MM.yyyy HH:mm')}</span>
              <span>•</span>
              <span>{formatChairAssistantChair(application.chair)}</span>
              <span>•</span>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.35fr_0.65fr]">
          <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-2xl">
            <div className="mb-5 flex items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Başvuru Formu</p>
            ...
            <div className="grid gap-4">
              {chairAssistantQuestions.map((question) => (
                <div
                  key={question.key}
          ...
          <aside className="space-y-6">
            <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-2xl">
              <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Takip</p>
              <h2 className="mt-2 text-xl font-semibold">Durum ve admin notu</h2>

              <form action={updateChairAssistantApplicationAction} className="mt-5 space-y-3">
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-2xl">
              <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Yüklenen Evraklar</p>
              <h2 className="mt-2 text-xl font-semibold">Belgeler</h2>

              <div className="mt-5 space-y-3">
                {chairAssistantDocumentDefinitions.map((definition) => {
                  const document = documentMap.get(definition.type)
                  return (
                    <div
                      key={definition.type}
                      className="rounded-2xl border border-slate-800 bg-slate-950/50 p-4"
                    >
                      ...
                      <p className="mt-1 text-xs text-slate-400">
                        {document?.fileName || 'Belge yüklenmemiş'}
                      </p>
                      {document ? (
                        <a
